            ...
            <div className="flex-1 h-2 bg-surface-high rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${
                  d.direction === "positive" ? "bg-primary-fixed" :
                  d.direction === "negative" ? "bg-cyber-red" : "bg-on-surface-variant"
                }`}
                style={{ width: `${Math.abs(d.contribution) * 100}%` }}
              />
            </div>
            <span className={`text-xs w-16 text-right font-mono ${
              d.direction === "positive" ? "text-primary-fixed" :
              d.direction === "negative" ? "text-cyber-red" : "text-on-surface-variant"
            }`}>
