...
// Content width: A4 (11906 DXA) - 2x1440 margins = 9026 DXA
const PAGE_W = 9026;
...
function cell(text, { bg = CARD_BG, bold = false, color = LIGHT, align = AlignmentType.LEFT, width } = {}) {
      ...
      ...rows.map((row, ri) =>
        new TableRow({
          children: row.map((val, ci) => {
            const bg = ri % 2 === 0 ? CARD_BG : "1A2942";
...
function body(text, { bold = false, color = LIGHT, italic = false } = {}) {
  return new Paragraph({
    spacing: { after: 120 },
    children: [new TextRun({ text, bold, italic, color, font: "Calibri", size: 20 })]
  });
}
...
function callout(text) {
  return new Paragraph({
    ...
    border: {
      left:  { style: BorderStyle.SINGLE, size: 20, color: VIOLET, space: 8 },
      right: { style: BorderStyle.SINGLE, size: 20, color: VIOLET, space: 8 },
      top:   { style: BorderStyle.SINGLE, size: 4,  color: SLATE,  space: 4 },
      bottom:{ style: BorderStyle.SINGLE, size: 4,  color: SLATE,  space: 4 },
    },
    ...
    config: [
      {
        ...
        levels: [{
          ...
          style: {
            paragraph: { indent: { left: 640, hanging: 360 }, spacing: { after: 80 } },
    ...
    // ══════════════════════════════════════════════════════════════════
    {
      ...
      children: [
        ...
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { after: 160 },
          children: [new TextRun({ text: "NeuralQuant", bold: true, font: "Calibri", size: 80, color: VIOLET })]
        }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { after: 120 },
        ...
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "\u26A0 CONFIDENTIAL \u2014 Internal Strategy Document", bold: true, font: "Calibri", size: 20, color: AMBER })]
        }),
    ...
    // ══════════════════════════════════════════════════════════════════
    {
      ...
      children: [
        ...
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { after: 160 },
          children: [new TextRun({ text: "Table of Contents", bold: true, font: "Calibri", size: 34, color: VIOLET })]
        }),
        ...
        spacer(),
        callout("Core thesis: The gap in the market is not data \u2014 Bloomberg and FactSet have data. The gap is affordable, explainable, AI-debated conviction. NeuralQuant fills that gap."),
        new Paragraph({ children: [new PageBreak()] }),

        // ── 2. COMPETITIVE LANDSCAPE ──────────────────────────────────
        heading1("2. Competitive Landscape"),
        body("The following table benchmarks NeuralQuant against eight established competitors across key dimensions that matter most to retail and professional investors."),
        spacer(),
        makeTable(
          ["Platform", "AI Analysis", "Quant Model", "Live Data", "India", "Price/mo", "Explainable"],
          [1500, 1300, 1200, 1100, 700, 900, 1326],
          [
        ...
        spacer(),
        heading2("NeuralQuant vs Perplexity Finance"),
        body("Perplexity Finance is a general-purpose LLM with financial news access. It cannot run a screener, produce a factor score, or cite proprietary signals. NeuralQuant\u2019s query engine injects live macro data (VIX, HY spreads, CPI, Fed funds), screener rankings, and 52-week price ranges before every LLM call \u2014 making every answer data-grounded and citable."),

        heading2("NeuralQuant vs Danelfin"),
        body("Danelfin scores stocks using 100+ ML features but operates as a black box \u2014 users cannot see why NVDA scores 8/10. NeuralQuant exposes all five factor percentiles, the regime label, and confidence \u2014 giving actionable conviction rather than an opaque number."),

        heading2("NeuralQuant vs Bloomberg Terminal"),
        body("Bloomberg provides the deepest institutional data at $2,000+/month per seat \u2014 inaccessible to retail investors. NeuralQuant delivers 80\u201390% of actionable insight (live macro, fundamentals, factor scores, AI synthesis) at 1\u201350% of the cost."),
        ...
        body("No direct competitor at this price point covers NSE stocks with the same depth. NeuralQuant scores 50 NSE stocks with the same 5-factor model, regime detection, and PARA-DEBATE analysis. India\u2019s 90M+ Demat account market has no equivalent tool in this price range."),

        heading2("USP 6 \u2014 Full Explainability at Every Layer"),
        body("Every 1\u201310 score shows the five underlying factor percentiles. Every PARA-DEBATE analysis shows each agent\u2019s perspective. Every NL query response cites its data sources. No black boxes \u2014 this builds user trust and supports investment decision documentation for RIAs."),

        new Paragraph({ children: [new PageBreak()] }),

        // ── 4. DRAWBACKS ──────────────────────────────────────────────
        heading1("4. Known Drawbacks & Honest Limitations"),
        body("A credible business document must acknowledge what the platform does not yet do well. All limitations below have clear Phase 4/5 fixes."),
        spacer(),
        makeTable(
        ...
        body("RIAs need to document their investment thesis. NeuralQuant\u2019s PARA-DEBATE output gives them a structured, citable analysis per stock \u2014 MACRO, FUNDAMENTAL, TECHNICAL, SENTIMENT, GEOPOLITICAL perspectives \u2014 that can be included in client reports. At $49\u2013$99/month, it costs less than one hour of a junior analyst\u2019s time."),

        heading2("For the Quant Researcher"),
        body("The /screener and /stocks API endpoints expose raw factor percentiles, composite scores, regime IDs, and confidence as JSON. Quant researchers can pipe NeuralQuant\u2019s signals into their own models, backtest regime-conditional strategies, or use India NSE data as a diversification signal."),

        heading2("For the Indian Investor"),
        body("The NSE market is underserved by Western analytics tools. NeuralQuant scores 50 top NSE stocks with the same rigour as US equities \u2014 live yfinance fundamentals, Jegadeesh-Titman momentum, P/B value, and short-interest signals \u2014 plus a query engine that understands Indian market context."),
        ...
        bullet("Subscription SaaS \u2014 primary revenue; recurring, predictable, high gross margin"),
        bullet("API Usage Overage \u2014 $0.01/call above tier limit; scales naturally with power users"),
        ...
        heading2("6.3 Revenue Projections (Conservative)"),
        body("Assumes 5% monthly user growth, 8% free-to-paid conversion, and $30 average ARPU at Month 24:"),
        spacer(),
        makeTable(
          ["Month", "Free Users", "Paid Users", "Avg. ARPU", "MRR", "ARR Run-Rate"],
          [900, 900, 900, 1000, 1200, 4126],
          [
        ...
        spacer(),
        callout("At Month 24, a $1M ARR run-rate at 85% gross margin positions NeuralQuant for a Seed or Series A raise at a 5-10x ARR multiple ($5-10M valuation)."),
        spacer(),
        ...
        bullet("CAC: $15\u201325 via content marketing + SEO + community"),
        bullet("LTV at $30 ARPU, 18-month avg. retention: $540 \u2014 LTV:CAC ratio of ~25x"),
        ...
        body("Target finance-forward communities: r/IndiaInvestments, r/stocks, r/quantfinance, Substack finance newsletters, and Indian stock trading Telegram groups. Release a free tier with generous limits. Publish weekly \u2018NeuralQuant Weekly\u2019 reports showcasing the PARA-DEBATE output on trending stocks. Goal: 1,000 free signups, 50 paid conversions."),

        heading2("Phase 2: Paid Conversion Engine (Months 4\u20139)"),
        body("Gate PARA-DEBATE behind the Investor tier. Use the free tier as a loss leader \u2014 users who see a PARA-DEBATE preview will convert to unlock the full debate. Partner with mid-sized Indian stock trading communities for co-branded content. Launch an affiliate program. Goal: 200 paid users, $5,000 MRR."),
        ...
        body("Technology moats in AI products compound over time. NeuralQuant\u2019s defensibility comes from five overlapping sources:"),
        spacer(),
        makeTable(
        ...
        spacer(),
        body("The largest risk is not technical \u2014 it is distribution. The platform needs to reach the communities that will benefit from it most. With a well-executed free tier, a content-led growth motion, and a clear API monetisation path, NeuralQuant is positioned to reach $1M ARR within 24 months."),
        spacer(),
        ...
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "Prepared by: NeuralQuant Engineering Team  \u00B7  March 2026  \u00B7  CONFIDENTIAL", font: "Calibri", size: 18, color: MID_GREY, italics: true })]
        }),
      ]
    }
  ]
